...
/** Integer split of `total` across parts proportional to non-negative `weights`. */
...
/**
 * Mutates `plan.mealsByDay` in place when `plan.macroTargets` is present.
 * Each day's summed protein/carbs/fat match targets; kcals are scaled to hit calorie target
 * after P/C/F are fixed (small drift from strict 4/4/9 per meal is acceptable).
 */
