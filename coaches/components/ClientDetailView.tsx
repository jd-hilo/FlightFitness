...
type Tab = "overview" | "meals" | "workouts" | "reflections" | "messages";
...
export function ClientDetailView({
  ...
  const [promptTitle, setPromptTitle] = useState("Reflection");
  ...
  const loadPrompts = useCallback(async () => {
    try {
      const res = await fetch(`/api/clients/${clientId}/reflections`, {
        credentials: "include",
      });
      const json = (await res.json()) as { prompts?: ReflectionPrompt[]; error?: string };
      if (res.ok) setPrompts(json.prompts ?? []);
    } catch {
      /* ignore */
    }
  }, [clientId]);
            ...
            <div className="space-y-4">
              <div className="border border-zinc-800 rounded-lg p-4 bg-zinc-900/40 space-y-3">
                <p className="text-xs uppercase tracking-wider text-amber-400 font-semibold">
                  Assign prompt (shows on Faith)
                </p>
                <label className="block text-xs text-zinc-400">
                  Date
                  <input
                    type="date"
                    className="block w-full mt-1 bg-zinc-950 border border-zinc-700 rounded px-2 py-1.5 text-sm"
                    value={promptDate}
                ...
                <label className="block text-xs text-zinc-400">
                  Prompt
                  <textarea
                    className="block w-full mt-1 bg-zinc-950 border border-zinc-700 rounded px-2 py-1.5 text-sm min-h-[100px]"
                    value={promptBody}
                    onChange={(e) => setPromptBody(e.target.value)}
                    placeholder="What should they sit with today?"
                  />
                </label>
                <button
                  type="button"
                  ...
                  className="px-4 py-2 rounded-md bg-amber-500 text-black font-semibold text-sm disabled:opacity-50"
                >
              ...
              <ul className="space-y-2">
                {prompts.length === 0 ? (
                  <p className="text-zinc-500 text-sm">No reflection prompts yet.</p>
                ) : (
                  prompts.map((p) => (
                    <li
                      key={p.id}
                    ...
                    >
                      <p className="text-[10px] uppercase tracking-wide text-amber-400">
                        {p.prompt_date} · {p.title}
                      </p>
                      <p className="text-sm text-zinc-200 mt-1 whitespace-pre-wrap">{p.body}</p>
                    </li>
                  ))
                )}
              </ul>
